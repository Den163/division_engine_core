...
void division_engine_internal_platform_render_pass_context_free(DivisionContext* ctx)
{
    for (int i = 0; i < ctx->render_pass_context->render_pass_count; i++)
    {
        DivisionRenderPassInternalPlatform_* pass =
            &ctx->render_pass_context->render_passes_impl[i];
...
)
{
    DivisionRenderPassInternalPlatform_* render_pass_impl =
        &ctx->render_pass_context->render_passes_impl[pass_id];
...
)
{
    ...
    DivisionRenderPassInternalPlatform_* render_pass_impl =
        &render_pass_ctx->render_passes_impl[render_pass_id];
    DivisionMetalShaderProgram* shader_program =
        &ctx->shader_context->shaders_impl[render_pass->shader_program];
    ...
    if (division_utility_mask_has_flag(
            render_pass->capabilities_mask, DIVISION_RENDER_PASS_CAPABILITY_ALPHA_BLEND
        ))
    {
...
MTLColorWriteMask division_to_mtl_color_mask(DivisionColorMask color_mask)
{
    ...
    result_mask |=
        (MTLColorWriteMaskRed *
         division_utility_mask_has_flag(color_mask, DIVISION_COLOR_MASK_R));
    result_mask |=
        (MTLColorWriteMaskGreen *
         division_utility_mask_has_flag(color_mask, DIVISION_COLOR_MASK_G));
    result_mask |=
        (MTLColorWriteMaskBlue *
         division_utility_mask_has_flag(color_mask, DIVISION_COLOR_MASK_B));
    result_mask |=
        (MTLColorWriteMaskAlpha *
         division_utility_mask_has_flag(color_mask, DIVISION_COLOR_MASK_A));
