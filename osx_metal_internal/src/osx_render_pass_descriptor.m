...
static inline bool try_get_mtl_blend_op(
    DivisionContext* ctx,
    DivisionAlphaBlendOperation blend_op,
    MTLBlendOperation* out_mtl_blend_op
);
...
void division_engine_internal_platform_render_pass_context_free(DivisionContext* ctx)
{
    ...
    {
        DivisionRenderPassInternalPlatform_* pass =
            &ctx->render_pass_context->render_passes_descriptors_impl[i];
        pass->mtl_pipeline_state = nil;
    }
    free(ctx->render_pass_context->render_passes_descriptors_impl);
}

void division_engine_internal_platform_render_pass_free(
    DivisionContext* ctx, uint32_t pass_id
)
{
    DivisionRenderPassInternalPlatform_* render_pass_impl =
        &ctx->render_pass_context->render_passes_descriptors_impl[pass_id];
...
)
{
    ...
    render_pass_ctx->render_passes_descriptors_impl = realloc(
        render_pass_ctx->render_passes_descriptors_impl,
        sizeof(DivisionRenderPassInternalPlatform_[new_size])
    );

    return render_pass_ctx->render_passes_descriptors_impl != NULL;
}
...
)
{
    ...
    DivisionRenderPassInternalPlatform_* render_pass_impl =
        &render_pass_ctx->render_passes_descriptors_impl[render_pass_id];
    DivisionMetalShaderProgram* shader_program =
        &ctx->shader_context->shaders_impl[render_pass_desc->shader_program];
    DivisionVertexBufferInternalPlatform_* vert_buff =
        &ctx->vertex_buffer_context->buffers_impl[render_pass_desc->vertex_buffer_id];
    DivisionOSXViewDelegate* view_delegate =
        ctx->renderer_context->window_data->app_delegate->viewDelegate;
    DivisionAlphaBlendingOptions* blend_options =
        &render_pass_desc->alpha_blending_options;
    ...
    id<MTLRenderPipelineState> pipeline_state =
        [view_delegate->device newRenderPipelineStateWithDescriptor:pipeline_descriptor
                                                              error:&err];

    if (err)
    {
        DIVISION_THROW_INTERNAL_ERROR(ctx, [[err debugDescription] UTF8String]);
        return false;
    }

    if (!pipeline_state)
    {
        DIVISION_THROW_INTERNAL_ERROR(ctx, "Render pipeline state is null");
        return false;
    }
...
)
{
    ...
    case DIVISION_ALPHA_BLEND_ONE_MINUS_CONSTANT_ALPHA:
        *out_mtl_blend_arg = MTLBlendFactorOneMinusBlendAlpha;
        return true;
    default:
        DIVISION_THROW_INTERNAL_ERROR(ctx, "Unknown Blend to MTLBlendFactor mapping");
        return false;
    }
}
...
)
{
    ...
    default:
        DIVISION_THROW_INTERNAL_ERROR(
            ctx, "Unknown BlendOperation to MTLBlendOperation mapping"
        );
        return false;
    }
}
