...
void division_engine_internal_platform_uniform_buffer_context_free(DivisionContext* ctx)
{
    ...
    DivisionUniformBufferInternal_* uniform_buffers_impl =
...
{
    return [ctx->uniform_buffer_context->uniform_buffers_impl[buffer_id]
                .mtl_buffer contents];
}
...
)
{
    ...
    id<MTLBuffer> mtl_buffer =
        uniform_buffer_ctx->uniform_buffers_impl[buffer_id].mtl_buffer;
...
{
    DivisionUniformBufferInternal_** uniform_buffers_impl =
        &ctx->uniform_buffer_context->uniform_buffers_impl;

    *uniform_buffers_impl =
        realloc(*uniform_buffers_impl, sizeof(DivisionUniformBufferInternal_[new_size]));
...
)
{
    ...
    DivisionUniformBufferDescriptor* uniform_buffer =
    ...
    if (mtl_buffer == nil)
    {
        ctx->lifecycle.error_callback(
            ctx, DIVISION_INTERNAL_ERROR, "Failed to create MTLBuffer"
        );
        return false;
    }
