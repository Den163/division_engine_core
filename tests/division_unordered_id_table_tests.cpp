...
TEST_CASE("Unordered id table insert check")
{
    ...
    uint32_t id = division_unordered_id_table_new_id(&table);
    REQUIRE(id == 0);
...
TEST_CASE("Unordered id table remove check")
{
    ...
    uint32_t id = division_unordered_id_table_new_id(&table);
    division_unordered_id_table_remove_id(&table, id);
...
TEST_CASE("Unordered id table mixed operations") {
    ...
    uint32_t id0 = division_unordered_id_table_new_id(&table);
    uint32_t id1 = division_unordered_id_table_new_id(&table);
    uint32_t id2 = division_unordered_id_table_new_id(&table);
    uint32_t id3 = division_unordered_id_table_new_id(&table);

    division_unordered_id_table_remove_id(&table, id2);
