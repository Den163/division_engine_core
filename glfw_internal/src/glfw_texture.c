...
static inline bool get_texture_traits(
    DivisionContext* ctx, DivisionTextureFormat texture_format, GlTextureTraits_* out_traits);
...
bool division_engine_internal_platform_texture_impl_init_new_element(DivisionContext* ctx, uint32_t texture_id)
{
    ...
    glTextureStorage2D(
        tex_impl->gl_texture, 1, gl_traits.gl_sized_internal_format, (GLsizei) tex->width, (GLsizei) tex->height);
...
void division_engine_internal_platform_texture_set_data(DivisionContext* ctx, uint32_t texture_id, void* data)
{
    ...
    glTextureSubImage2D(
        ...
        0,
        0, 0,
        (GLsizei) tex->width, (GLsizei) tex->height,
        tex_impl->gl_texture_format, GL_UNSIGNED_BYTE,
        data
    );
}
